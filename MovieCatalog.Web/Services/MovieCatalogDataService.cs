...
public class MovieCatalogDataService : IMovieCatalogDataService
{
    ...
    {
        return await ctx.Genres.Select(g => new {
            Genre = g,          
            TitleCount = g.TitleGenres.Count  
        })
        .ToDictionaryAsync(g => g.Genre, g => g.TitleCount);
    ...
    {
        var title = await ctx.Titles.Where(t=>t.Id == id).Include(t=>t.TitleGenres).FirstOrDefaultAsync();
        if(title == null)
        {
